...
### mysql
你的 MySQL 容器名为 mysql8，root 密码是 root。以下是完整操作步骤：
...
-- 创建 website 用户
CREATE USER IF NOT EXISTS 'website'@'%' IDENTIFIED BY 'website1020';

-- 授予该用户对 nacos_config 库的全部权限
GRANT ALL PRIVILEGES ON *.* TO 'website'@'%';
...
镜像用的是 rabbitmq:3.13-management（第 3 行）
带 -management 后缀的官方镜像已经内置并默认启用了管理插件（rabbitmq_management），这就是可视化控制台。
如果用的是不带后缀的 rabbitmq:3.13，则没有控制台。
已映射控制台端口 15672（第 8 行）
已设置管理账号（admin/admin123）
...
nacos配置sentinel 限流，熔断，授权，用户限流规则
参见docker-compose/sentinel目录下 sca-web-sentinel-degrade.json、sca-web-sentinel-flow.json、sca-web-sentinel-authority.json和sca-web-sentinel-userflow.json
...
用户限流规则（防刷，自定义 UserFlowSlot）：基于 Sentinel ProcessorSlot SPI 扩展，按（资源，登录用户）窗口计数限流，单用户超限返回 429 + code 1012。规则配置在 Nacos（data-id: sca-web-sentinel-userflow，group: SENTINEL_GROUP），设计文档参见项目根目录《Sentinel自定义UserFlowSlot设计与实现.md》。

拦截审计（观测型 BlockAuditSlot）：sca-web 捕获限流/熔断/授权拦截事件后异步推 Kafka（topic-sentinel-block），由 sca-datasync 消费写入 ES 索引 sentinel_block_index，可按用户/接口/拦截类型聚合分析；需先启动 Kafka 与 ES，审计链路不可用时不影响业务（fail-open）。详见同一设计文档第 8 章。
